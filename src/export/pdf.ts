...
const LEGAL_HINT =
  "Open ArtCore — plantilla orientativa. Este documento no ha sido revisado por profesionales del derecho y no constituye asesoramiento legal.";
...
function fileBaseName(docTitle: string): string {
  const cleaned = docTitle
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 56);
  return cleaned || "documento-open-artcore";
}
...
<body>
  ...
  ${blocks}
  <p class="screen-page-note">Usar el diálogo de impresión del navegador para guardar como PDF. Si el navegador ofrece numeración de páginas en el pie, activarla ahí.</p>
</body>
</html>`;
}
