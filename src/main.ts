 ...
 *
 * Open ArtCore — web entry.
 * Flow: platform home (/) → app (/acuerdos-de-exhibicion) → wizard → review → accept → export.
...
function syncUrl(phase: AppPhase, replace = false): void {
  navigate(pathForPhase(phase, state.templateId), replace);
}
...
function enterTool(templateId: string, phase: ToolPhase = "wizard"): void {
...
/** Apply a parsed URL route (boot + browser back/forward). */
function applyRoute(route: Route, replaceInvalid = false): void {
  if (route.kind === "home") {
  ...
  if (route.kind === "unknown") {
    if (isToolPhase(state.phase)) state.lastToolPhase = state.phase;
    state.phase = "home";
    if (replaceInvalid) navigate("/", true);
    return;
  }

  const templateId = route.tool.templateId;
  if (!templateId || route.tool.status !== "available") {
    state.phase = "home";
    if (replaceInvalid) navigate("/", true);
    return;
  }
  ...
  if (
    ...
    (isToolPhase(state.phase) || hasDocumentWork(state, DEFAULT_TOGGLES))
  ) {
    state.phase = isToolPhase(state.phase)
      ? state.phase
      : (state.lastToolPhase ?? "wizard");
    return;
  }

  state = createEmptySession(templateId, DEFAULT_TOGGLES, state.personalProfile);
  state.phase = "wizard";
  state.stepIndex = 0;
...
function downloadCurrentProfile(): void {
  ...
  if (!profile || !profileHasData(profile)) {
    alert(
      "No hay datos personales que guardar. Hay que rellenarlos abajo o cargar un archivo primero.",
    );
    return;
  }
...
function profileDisclosureHint(open: boolean): string {
  if (profileHasData(state.personalProfile)) {
    return profileLabel(state.personalProfile!);
  }
  return open ? "Sin datos aún" : "Opcional";
}

function profileStatusText(): string {
  return profileHasData(state.personalProfile)
    ? `En memoria ahora: ${profileLabel(state.personalProfile!)} (se borran al cerrar la pestaña).`
    : "Ningún dato en memoria. Se puede rellenar el formulario o cargar un archivo.";
}

/** Platform-level: personal data for any tool (collapsible). */
function renderPlatformProfile(): HTMLElement {
  const box = document.createElement("details");
  box.className = "oat-platform-profile";
  ...
  summary.setAttribute(
    "aria-label",
    "Datos personales. Panel opcional para cargar o editar el perfil.",
  );

  const chevron = el("span", "oat-disclosure-chevron");
  chevron.setAttribute("aria-hidden", "true");

  const titleWrap = el("span", "oat-disclosure-copy");
  const title = el("span", "oat-disclosure-title");
  title.textContent = "Datos personales";
  const hint = el("span", "oat-disclosure-hint");
  hint.textContent = profileDisclosureHint(box.open);
  titleWrap.append(title, hint);

  const meta = el("span", "oat-disclosure-meta");
  meta.textContent = box.open ? "Cerrar" : "Abrir";
  meta.setAttribute("aria-hidden", "true");
  summary.append(chevron, titleWrap, meta);
  ...
  note.textContent =
    "Datos personales para usarlos en cualquier herramienta de la plataforma. Viven en un .json que se descarga y se carga. La plataforma no los almacena. No es una agenda de clientes.";
  ...
  actions.append(
    btn("Cargar datos personales (.json)", "oat-btn oat-btn-ghost", () => {
      void pickProfileFile();
    }),
    btn("Descargar datos personales (.json)", "oat-btn oat-btn-ghost", downloadCurrentProfile),
  );
...
function renderToolDraftBar(): HTMLElement {
  const box = el("aside", "oat-file-bar");
  box.setAttribute("role", "note");
  const title = el("h3");
  title.textContent = "Borrador de este acuerdo";
  const note = el("p", "oat-review-note");
  note.textContent =
    "Solo de esta herramienta. Instantánea del formulario, cláusulas y opciones. Se puede descargar en cualquier momento desde la franja superior o aquí, y cargar para retomar este documento.";
  const actions = el("div", "oat-actions");
  actions.append(
...
function renderApplyPlatformProfile(): HTMLElement {
  ...
  const note = el("p", "oat-review-note");
  if (profileHasData(state.personalProfile)) {
    note.textContent = `Datos personales en memoria: ${profileLabel(state.personalProfile!)}. Se usan para rellenar la parte de autoría de este documento, no la de clientes.`;
    const actions = el("div", "oat-actions");
    actions.append(
      btn("Rellenar autoría con los datos personales", "oat-btn oat-btn-ghost", () => {
        applyAuthorFromProfile(state.personalProfile!);
        render();
      }),
    );
    ...
    note.textContent =
      "Se pueden cargar datos personales en la página de la plataforma (sirven para cualquier herramienta). Aquí solo se rellena la autoría de este documento.";
    box.append(note);
  }
  return box;
}
...
function render(): void {
  ...
  document.title = tool
    ...
    : `${PLATFORM.name} — plataforma open source para el arte`;
...
function renderHome(): HTMLElement {
  ...
  eyebrow.textContent = "Plataforma paraguas";
  const h1 = el("h1");
  ...
  originTitle.textContent = "De dónde sale";
  const originBody = el("p", "lede");
  originBody.textContent = PLATFORM.origin;
  ...
  for (const tool of TOOLS) {
    shelf.append(renderAppCard(tool));
  }
...
function renderSupport(): HTMLElement {
  ...
  if (donateUrl) {
    ...
    const note = el("p", "oat-review-note");
    note.textContent =
      "Se abre fuera de Open ArtCore. La cantidad la elige quien aporta. Se puede cancelar en cualquier momento antes de confirmar en el proveedor.";
    actions.append(note);
  } else {
    const pending = el("p", "oat-support-pending");
    pending.textContent =
      "Todavía no hay un enlace de pago público configurado. Quien quiera aportar puede contactar con Gerard Valls Montaño para saber cómo hacerlo con total claridad.";
...
function renderAccept(): HTMLElement {
  ...
  const note = el("p", "oat-review-note");
  note.textContent =
    "Hay que revisar el documento final. Si está correcto, se confirma y se exporta. Para PDF se usa el diálogo de impresión del navegador (se puede activar la numeración de páginas ahí si está disponible).";
